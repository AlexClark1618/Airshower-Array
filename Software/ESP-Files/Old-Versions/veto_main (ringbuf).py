...
def readData():
    ...
    try:
        ...
        packet = data_packing(send_packet_format, 100, mac_id, 9, 1, 0, 0, 0, 0, 0, 0)
        send_data(packet)   
        time.sleep(0.1)
        machine.reset()
...
while True:
    ...
    try:
        ...
        if diff > 5_000_000:
            ...
            data_msg = (97, mac_id, gc.mem_free(), gc.mem_alloc(), 0, 0, 0, 0, 0, 0)
        ...
        if tRaw1 is None:
            # no anchor this iteration -- skip the packing loop
            wdt.feed()
            continue
        ...
        for i in range(lenRaw):
            #Return all calibrated events on both channels
            #if chRaw[i] == 0:
