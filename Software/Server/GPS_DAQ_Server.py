import socket
import selectors #changed from select
import struct
import os
import re
import gzip
...
def cleanup_client_socket(sock, addr, sel, sockets, clients, clients_by_id):
    #                           ^^^ add sel parameter
    try:
        sel.unregister(sock)   # NEW: release the FD from the selector
    except Exception:
        pass                   # already unregistered, fine
    ...
    if client_info:
        ...
        del clients[sock]
    try:
        sock.close()           # wrap in try like you do in ctrl version
    except OSError:
        pass
...
def cleanup_ctrl_socket(sock, addr, sel, sockets, ctrl_clients, ctrl_by_id, pending_slow_ctrl):
    try:
        sel.unregister(sock)   # NEW
    except Exception:
        pass
...
def run_server():
    ...
    sel.register(ctrl_server, selectors.EVENT_READ)
    ...
    try:
        while True:
            ...
            readable = [key.fileobj for key, mask in events]
            ...
            for s in readable:
                if s is server:
                    try:
                        ...
                        sel.register(client_socket, selectors.EVENT_READ)  # NEW
                        sockets.append(client_socket)
                        clients[client_socket] = {"buffer": bytearray(), "addr": addr}
                    ...
                    try:
                        ...
                        sel.register(ctrl_socket, selectors.EVENT_READ)    # NEW
                    ...
                    try:
                        data = s.recv(2048)
                        if not data:
                            cleanup_ctrl_socket(s, caddr, sel, sockets, ctrl_clients, ctrl_by_id, pending_slow_ctrl)
                            continue
                    except OSError as e:
                        if e.errno in (errno.EAGAIN, errno.EWOULDBLOCK):
                            continue
                                ...
                                if ID == GUI_ID:
                                    ...
                                    if target_sock:
                                        ...
                                        try:
                                            ...
                                            cleanup_ctrl_socket(target_sock, t_addr, sel, sockets, ctrl_clients, ctrl_by_id, pending_slow_ctrl)
                                    else:
                    ...
                    try:
                        ...
                        if not data:  # empty = client closed connection
                            cleanup_client_socket(s, client_addr, sel, sockets, clients, clients_by_id)
                            continue
                    ...
                    while len(client_info["buffer"]) >= PACKET_SIZE:
                        if clients.get(s) is not client_info:
                            print(f"[DIVERGE] {client_addr}")
                        ...
                        if client_info.get("id") != ID:
                            ...
                            if existing_sock is not None and existing_sock != s:
                                ...
                                cleanup_client_socket(existing_sock, old_addr, sel, sockets, clients, clients_by_id)
                            clients_by_id[ID] = s
                            client_info["id"] = ID
                                        ...
                                        if client_ID not in (128, 16, GUI_ID):
                                            ...
                                            try:
                                                ...
                                                if e.errno in (errno.EAGAIN, errno.EWOULDBLOCK):
                                                    ...
                                                    cleanup_client_socket(client_sock, target_addr, sel, sockets, clients, clients_by_id)
                            ...
                            if ID == GUI_ID:
                                ...
                                if target_sock:
                                    ...
                                    try:
                                        ...
                                        t_addr = t_info["addr"] if t_info else "Unknown"
                                        cleanup_client_socket(target_sock, t_addr, sel, sockets, clients, clients_by_id)
                                else:
